...
export function LawyerPerformanceChart() {
  ...
  const lawyerStats = mockLawyers.map((lawyer) => {
    const assignedCases = mockCases.filter(
      (c) => c.assignedLawyerId === lawyer.id
    );
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="text-xl font-bold text-foreground">
          {t("charts.lawyerPerformance.title")}
        </h3>
      </div>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={lawyerStats} barGap={8}>
          ...
          <Bar
            dataKey="total"
            fill="#dc2626"
            name={t("charts.lawyerPerformance.legend.total")}
            radius={[8, 8, 0, 0]}
          />
          <Bar
            dataKey="completed"
            fill="#10b981"
            name={t("charts.lawyerPerformance.legend.completed")}
            radius={[8, 8, 0, 0]}
          />
          <Bar
            dataKey="active"
            fill="#3b82f6"
            name={t("charts.lawyerPerformance.legend.active")}
            radius={[8, 8, 0, 0]}
          />
        </BarChart>
      </ResponsiveContainer>
